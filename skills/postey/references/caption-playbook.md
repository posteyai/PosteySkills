...
7. **Verify and validate before presenting.** After creating a draft, fetch it back per platform: read `postey://posts/{id}/content/{platform}`, or call `get_specific_post_content` if your client cannot read resources. Then check: caption present, media attached and correct, right account, right format (single post vs thread). Also run the `validate_post_content` tool per platform. Fix failures before presenting; never present a draft with known issues.
