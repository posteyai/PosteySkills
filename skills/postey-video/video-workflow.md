...
**Step 2** — Generate polished captions from the `transcript` field using the rules in the hub skill's `prompts.md` (it ships with `postey`, which this pack requires). Apply the rules yourself — never paste the raw transcript as a caption.
